...
class ImageHandler(FileSystemEventHandler):
    def __init__(self, processor, model, device, output_file):
        self.processor = processor
        self.model = model
        self.device = device
    ...
    def on_created(self, event):
        if event.is_directory:
            ...
            caption = describe_image(event.src_path, self.processor, self.model, self.device)
            if caption:
                self.save_to_json(event.src_path, caption)

    def save_to_json(self, image_path, caption):
        data = []
...
if __name__ == "__main__":
    ...
    directory_to_watch = r"C:\Users\lebed\PycharmProjects\AI glasses\photos"
    output_file = 'captions.json'

    event_handler = ImageHandler(blip_processor, blip_model, device, output_file)
    observer = Observer()
